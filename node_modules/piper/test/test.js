describe("piper", function(){
    var p, piper, expect, one, number, zero;
    expect = require('expect.js')
    number = require(__dirname + '/../../rats/index.js')
    p = piper = require(__dirname + '/../index.js')
    function riser(to){
        var a = [], i;
        for ( i=0; i<to-1; i++ ){ a[i] = number(0) }
        a[i] = number(1)
        return a
    }
    ...
    it("basic operations should work", function(){
        ...
        expect(p([two]).pow(2)).to.equal(p([4]))
        expect(p([five]).pow(3)).to.equal(p([five.times(five).times(five)]))
        expect(p([five]).times(p([five.times(five)]))).to.equal(p(cube.val([five])))
