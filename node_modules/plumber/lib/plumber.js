...
void function(root){
    ...
    function op(method, input, input2){
        ...
        var len, result = [], i, j, l, r, p, plen, q, qlen;
        ...
        if ( method === 'plus' ) {
            ...
            result = plumber(0)
            len = input.ixs.length;
            if ( input2.typeLvl == null || input2.typeLvl < 3 ) input2 = plumber(input2)
            for ( i=0; i < len; i++ ) {
                result = result.plus(plumber(input.ixs[i]).times(input2.pow(i)))
            }
        } else if ( method === 'compose' ) {
            return BiPolyRat(input,input2)
        } else if ( method === 'gt' ) {
            return
        } else if ( method === 'per' ) {
    ...
    var PolyInt = luma.factory({
        ...
        , leftTr : function (input){
            return op('val',this,plumber([input,1]))
        }
    ...
    var PolyRat = luma.factory({
        init: function(input,input2,s) {
            var denominator=plumber(input2), numerator=plumber(input), divisor
                , n, t;
            if ( numerator.typeLvl < 3 || numerator.typeLvl > 5
                || denominator.typeLvl < 3 || denominator.typeLvl > 5 ){
    ...
    var BiPolyRat = luma.factory({
        init: function(input) {
            var i, j, degree, result=[], d
            , len = input.length;
            for ( i=0; i < len; i++ ) {
        ...
        , toString: function () {
            return "\n[\n"+
            this.ixs.reduce(function(p,c,i,a){
                return p+"\n"+c.toString()
            })+
            "\n[\n"
        }
