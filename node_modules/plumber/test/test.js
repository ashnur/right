describe("plumber", function() {
    ...
    it("rational polynumbers", function() {
        ...
        var tpr2 = p([8,10,-5,3]).per(p([-3,2,1]))
        var teszt1 = p([15,2,-5,11,4])
        var teszt2 = p([4,6,-1])
        console.log('t1 gcd ',p([1,0,-1]).divide(p([1,1]))+'')
        console.log('t2 gcd',p([1,4,6,4,1]).gcd(p([1,2,1]))+'')
        console.log('t3 gcd ',p([1,0,-1]).gcd(p([1,1]))+'')
        console.log('t4 div',teszt1.divide(teszt2)+'')
        console.log('t5 gcd',teszt1.gcd(teszt2)+'')
        ...
        var op1 = tpr.minus(tpr2)

        var op2 = p([-11,-17,12]).per(p([-3,2,1]))

        var op3 = p([-11,-17,12],p([-3,2,1]))

        console.log('minusteszt',tpr.minus(tpr2)+'',p([-11,-17,12]).per(p([-3,2,1]))+'')
        expect(tpr.minus(tpr2)+'').to.equal(p([-11,-17,12]).per(p([-3,2,1]))+'')
        console.log('szorzas teszt',tpr.times(tpr2)+'', p([8,34,25,-12,9]).per(p([-3,2,1]))+'')
        expect(tpr.times(tpr2)).to.equal(p([8,34,25,-12,9]).per(p([-3,2,1])))
        debugger
        console.log(tpr.per(tpr2)+'', p([-3,-7,7,3]).per(p([8,10,-5,3]))+'')
